...
class RelayGridOptionTests(unittest.TestCase):
    """The relay's grid is the library's time base now, behind one option.

    The classes it used to carry - ``Regridder`` for a chunk-stamped source and
    ``GridBuilder`` for a jittered one - are gone, along with the whole test class
    that exercised ``Regridder``: what they did is
    :class:`nova2026.streaming.TimeBase`, and ``test_timebase.py`` pins that
    behaviour on a chunk-stamped source and on a lost block. What is left to test
    here is the relay's own surface.
    """

    def parse(self, *extra: str):
        return relay.build_parser().parse_args(
            ["--source-name", "EE511", "--labels", "Fz", *extra]
        )

    def test_no_grid_unless_asked(self) -> None:
        self.assertFalse(
            self.parse().regrid, "the relay forwards untouched unless asked to regrid"
        )

    def test_both_spellings_select_the_same_mode(self) -> None:
        # --regrid-jitter named a mode of its own while a per-sample jittered
        # source needed one; the counted grid covers both shapes now.
        for flag in ("--regrid", "--regrid-jitter"):
            with self.subTest(flag=flag):
                self.assertTrue(self.parse(flag).regrid)

    def test_the_block_spreading_option_is_gone(self) -> None:
        # --regrid-lost-ratio described the algorithm TimeBase replaced. Accepting
        # it and ignoring it would be exactly the kind of leftover this migration
        # exists to remove.
        with self.assertRaises(SystemExit):
            self.parse("--regrid-lost-ratio", "1.5")

    def test_the_rate_option_still_sets_the_grid_rate(self) -> None:
        args = self.parse("--regrid", "--regrid-rate", "250")
        self.assertEqual(args.regrid_rate, 250.0)
