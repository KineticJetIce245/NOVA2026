...
| 5.5 | **偏移扫描实验** | `results/aad_shift_sweep_<date>.json`：包络滑 ±300 ms 的相关衰减曲线 | 扫描脚本 + 曲线 | 30 / 40 | **未完成（第 1 次尝试 `6ee0ba5`）** — 零点**逐位复现**步骤 5（`0.6194721652821231` / `0.5974097124824684`，窗口数 14528 与 `class_windows {0:9568, 1:4960}` 均相同），但**非零偏移点被证明不可信**：① 合成夹具在真实 +0.25 s 处本应为 1，实测 0.9652，峰值落在 0.225–0.2375 s → **约 1 个样本的系统错位**；② `--audit` 用原始链路 `replay_windows(audio_offset=δ)` 交叉验证，零点一致而**非零点差 +0.09~+0.20**，远超噪声。曲线数字**存档但不得采信**；缺陷以 2 个 `expectedFailure` + 3 个真实失败（含守卫测试 `SystemExit not raised`）自证在案。已派第 2 个子 agent 钉死配对后重跑 |
