...
| D-42 | 09-14 | **变异测试的通用陷阱立规**：首次变异扫描报出的 **3 个"存活变异"是假象**——`.pyc` 陈旧（文件 mtime 分辨率 1 s，被改文件复用了缓存字节码）。变异驱动现在**每个变异前清 bytecode 缓存**。另一处更严重：上一轮遗留的 2 个"存活变异"**不是测试弱，而是测试什么都没做**——`BlockPathTests._deliver` 注入了一个**没有任何 `Acquire` 读取**的 `source._queue`，并预先设置了 `chunks` 在读之前就检查的 stop 事件，所以它**一个字节都没投递**，无论实现怎样都返回绿 | 变异测试的价值全在"能不能变红"；假存活与空投递会让人误以为有覆盖 | 只报"存活"而不追查原因 | **§6.4 追加要求**：报告存活变异前必须先确认该测试**确实执行了被测路径**（本轮已修：helper 改为注入 stub `Acquire` 并驱动真实生成器；重排测试断言投递块等于 `rows[:, 1:21]`；预热测试改为喂 25+4000+2000 样本，因为 HQ 转换器需要 ~7.4 s 预热） | `8885612` |
