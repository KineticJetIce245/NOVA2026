...
| D-60 | 09-14 | **契约分割的变异框架此前是空洞的——「3/3 变异被捕获」这个证据无效。** `tests/streaming/test_contract_split.py` 的 `MutationEvidenceTests` 用 `unittest.main(module=None, argv=['unittest','-v','GatedKeysTests.test_x'])` 跑子进程；**`module=None` 时裸的 `Class.test` 名字会被当成顶层模块解析**，于是子进程死于 `ImportError: Failed to import test module: GatedKeysTests`、`Ran 1 test in 0.000s`、返回码 1——**每一个变异都被记为 `caught=True`，但原因是它根本没跑起来。** 破绽在旧记录里一眼可见：三行全是 `rc=1` + 完全相同的 `['FAILED (errors=1)']` + 0.000s。已修：runner 改为 `unittest.main(module='tests.streaming.test_contract_split', …)`，并**新增第四个变异**专门钉住那条回归线（`the-gate-refuses-a-key-absent-from-both-sides`）。重生成后 `survivors: []`，四个变异给出**互不相同**的红签名：`failures=1` / `errors=1` / `failures=1` / `errors=4` | 这是本仓库**第五个**变异测试陷阱，也是**最危险的一类**：前四个（D-42 字节码缓存、D-44 `.pth` 导入路径、CRLF/LF 查找落空、动作名猜错）都让「变异没生效」看起来像「变异被抓」；**这一个更彻底——整个框架从来没跑过被测测试，却每轮都报告全捕获**。后果是实质的：裁决在决定「保留还是回退分割」时，把「它自己的变异框架 3/3 通过」列为支持保留的证据之一，**而那份证据是空的**。（保留的结论本身仍然成立，依据是另外两条独立证据：ANT 上实测的 116/116 拒绝与两条 provenance 字符串之差，以及「把副本的 `decoder.py` 还原成 HEAD 后那 4 个测试全绿」的 A/B——这两条不受本陷阱影响。） | ① 相信「survivors 为空」② 只看 `caught=True` 而不看**红签名的形状**（同一个 `errors=1` 出现在每个变异上就是警报）③ 只跑一次框架不复核它的输出 | **§6.4 追加要求（第五、六条）**：① 变异框架必须**打印子进程的失败签名**，且**不同变异必须给出不同的签名**——雷同的签名意味着框架没有真正区分它们；② 任何「N/N 被捕获」的结论，必须同时给出**每个变异的实际红输出**，只给计数不足以采信。本次同时补上了 D1 缺失的用例：`test_a_gate_key_absent_from_both_sides_is_accepted`（两边都缺 → 必须被接受）与 `test_the_absent_from_one_side_message_is_true_where_it_refuses`（只有一边缺 → 必须拒绝且报错为真），并实测：把那行改回旧形式 → 新测试 RED 且报出**与外部复核一字不差**的假消息 「the processing keys units are absent from one side」，复原后 sha256 逐字节相同 | 待提交 |
| D-61 | 09-14 | **ANT 上跑出决策了：281 个，之前是 0。** 根因不是模型不迁移——**分数一直是好的**（450 个窗口里 343 个、76.2% 的 |差值| 超过 0.05；中位 0.1028；p10 0.0336），**是决策在能用之前被丢掉**。两次真实运行的计数对照：`declared`（契约修复前）`scoring_failures` 477、0 决策；`split`（契约修复后）打分成功但 `evidence_gap` 53、0 决策；**修复后 `evidence_gap` 0、`scoring_failures` 0、决策 281（A 155 / B 126 / 弃权 452）、failed 0、窗口 181**。改动只有两个从未被测量过的默认值：`Controller.max_age` 3.0 → 10.0 s（实测证据在实时路径上**真的老 2–4 秒**，3 秒的守卫在拒绝真实存在的证据）与 `offload_capacity` 2 → 8（116 个提交的窗口里丢了 53 个）。测量到的机制：**63 个窗口打了约 125 秒的钟 → 每个窗口约 2 秒打分，而 hop 只有 1 秒**，队列永久饱和、证据成串到达，年龄在 3 秒线附近震荡 | 用户的目标从「能不能跑」变成了「我要它下决定」。此前所有文档（含我写的）都把 0 决策归因为「模型不迁移」——**那是错的**，而且这个错误归因会让人去训新模型，而真正的问题是两个常量的量级 | ① 继续相信「参考电极不匹配 → 必须重训」（会去花几小时训模型而问题不在那）② 直接调低 margin 换取更多决策（买的更多但不是更准）③ 把 `max_age` 守卫删掉（守卫是对的：拒绝过期证据是正确行为；错的是**喂给它的证据真的过期了**，所以要修的是吞吐）  | **验证方式是真数据走真路径**：`scripts/getlive/ant_publish.py` 把操作者真实录制的 `session_19-34-06.npz` 按真实速率（500 Hz、20 电极、µV、峰值 83333 µV 的贴轨 F8）通过 LSL 广播，`scripts/auditory_ui.live` 走实时路径消费它——**不需要放大器**。运行记录 `results/live_demo_20260913-105725.{json,md}`。记录自己写明：**「Does not prove: that the decision is right.」**——模型是按 Cz/128 Hz 拟合的，这台设备是 CPz/500 Hz，所以**决策的正确性仍未测**，要用会话自己的标记标签去比。**另记两个未解释的异常**（同一份记录里）：`peak grid deviation 542.3 s`（一个 190 秒的运行）与 500→128 Hz 转换的样本缺口，两者都可能是残留延迟的一部分，都不理解 | 待提交 |
| D-62 | 09-14 | **500→128 Hz 少掉的 203 个样本不是速率偏差，是流末尾没被取出——而且我把它误判成了「整场错位 1.6 秒」。** 真实病因：`Resampler.__call__` 永远传 `last=False`（`src/nova2026/streaming/preprocess/resample.py:359-362`），类里没有任何途径索取 SoXR 缓冲里的尾巴；`ant_source.chunks()` 拆源时也不 drain，于是缓冲区随流丢弃。**样本从来没丢，是没人去取。** 决定性测量：每块输出分布是 **99.2% 的块输出为零**、然后以 845/846 整批到来（均值 6.3457），**这一条就否掉了「逐块取整」**（那会给出 22392）；defi​cit 前载、**峰值 947.40**（= 7.400 s × 128 Hz，正是运行时警告过的 HQ 启动延迟）**随后衰减**到 202.80——**真速率偏差会持续增长，实测是先达峰再衰减**。收尾 flush 精确找回 203：23885 = round(93300 × 128/500)，在 5 种几何下都精确成立（含短尾块与低于 HQ 延迟的退化情形，后者未 flush 时输出 **0**）。修法（形状 B，可证明的速率精确）：新增 `Resampler.drain()`（幂等、尾巴接在连续网格上）；`reset()` 改为重建 stream 而非 `clear()`（SoXR 在 `last=True` 后拒绝输入，实测 `RuntimeError: Input after last input`）；`chunks()` 在 `_close()` 前 yield 尾巴。**没动 `quality="HQ"` 及其预算**——换预设只改变缓冲量，不阻止未 flush 的样本被丢弃，那是掩盖不是修复。端到端复测：deficit **202.8 → 0**，比率 0.253826 → **0.25599791**（`results/live_demo_20260913-125339.json`） | **这条最重要的是我错在哪里。** 我在 D-61 和给用户的答复里把 deficit 说成「链的时钟比源慢 1.58 秒 → 整场错位 → 可能毁掉那 281 个决策」。**那个说法不是数据支持的形态。** 链的时间戳整场都是**正确标注**的：`_emitted/rate` 把第一个输出样本钉在源时间 0.0 且从未漂移，所以每个窗口都带着自己的真实源时间；突发式输出正是「计数落后但映射不漂移」的原因。**实际后果只是末尾约 1.6 s 的 EEG（约 0.85%）被切掉**，约 185 s 之前的窗口不受影响。我从一个正确的数字（deficit 203）推出了一个错误的机制（连续漂移），原因是**只看了总数、没看分布**——总数对两种机制都成立，分布才区分它们 | ① 用「整场错位」去否定那 281 个决策（**会误杀**，而且会让人去重训一个没问题的模型）② 用拉伸时间戳让总数对上（把丢失藏起来，正是被禁止的）③ 换掉 HQ 预设（掩盖而非修复）  | 缺陷本身是真的、修复是真的、deficit 归零是真的。**但「决策为什么不可信」这个问题没有被这条修复回答**，提出者明确拒绝给出它答不了的结论。它同时标出一个**更可疑、且尚未调查**的候选：**三份运行记录里 `grid_deviation_peak_seconds` 都是 542.3 / 491.7 / 556.3 秒**，而同一份记录里 `raw_rate_hz` 是 500.0000、`gaps` 为 0——这看起来是一个**独立的时基测量缺陷**，值得单独立项。另一个（按仓库自己的数字更可能主导结果）仍是 CPz 对 Cz 参考不匹配 + 缺 44 个电极。**§6.4 追加要求**：当一个总数（deficit、计数、比率）既支持「漂移」也支持「截断」时，**必须测分布或时序形状**再下结论；**总数不足以区分机制** | 待提交 |
