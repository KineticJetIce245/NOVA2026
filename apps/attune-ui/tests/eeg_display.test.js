...
const payload = () => ({ sample_rate: 16, channels: ['Cz'], samples: [[0, 60, -60, 30]], filtered_samples: [[0, 8, -4, 2]], filtered_sample_rate: 8,
...
test('EEG-F01 eeg_display decodes every frozen two-trace field unchanged', () => {
  assert.deepEqual(decoded(), { sampleRate: 16, channels: ['Cz'], samples: [[0, 60, -60, 30]], filteredSamples: [[0, 8, -4, 2]], filteredSampleRate: 8,
  ...
  assert.deepEqual(decodePacket(packet({ sample_rate: 32, channels: ['one'], samples: [[0, -1, .3]] })).values,
...
test('EEG-F02 wrong-typed fields decode to null per field, without coercion or throwing', () => {
  ...
  for (const [key, value] of bad) {
    ...
    if (key !== 'samples') assert.deepEqual(values.samples, [[0, 60, -60, 30]]);
    if (key !== 'filtered_samples') assert.deepEqual(values.filteredSamples, [[0, 8, -4, 2]]);
  }
});

test('EEG-F03 half-range is the trace peak, floored and capped at that trace own hint', () => {
  assert.deepEqual(traceValues([[0, 1, -2]]), [0, 1, -2]);
  assert.equal(traceValues([[0, NaN]]), null);        // non-finite => Unavailable, never drawn as zero
  assert.equal(traceValues([[0, null]]), null);
...
test('EEG-F04 two traces in one svg: raw on top, band-passed below, each on its own rate', () => {
  ...
  // raw: 16 Hz, 5 s window => x = i*7.5; +60 uV at the hint => y pinned 34 px from the band centre (40)
  assert.equal(raw, '0,40 7.5,6 15,74 22.5,23');
  // band-passed: 8 Hz, same window => x = i*15 (using sample_rate would give 0,7.5,15,22.5); +8 uV of the ±20 uV hint
  assert.equal(filtered, '0,120 15,86 30,137 45,111.5');
  assert.notEqual(filtered, '0,120 7.5,86 15,137 22.5,111.5', 'the filtered trace must not be mapped with sample_rate');
...
test('EEG-F05 the drawn scale is the trace half-range, never a fixed illustrative ±1', () => {
  const traces = tracesOf(render(decoded({ samples: [[0, 9, -3]], original_scale_hint_uv: 60, filtered_samples: [[0, 2]], filtered_scale_hint_uv: 20 })));
  assert.equal(traces[0].points, '0,40 7.5,6 15,51.33');                       // ±9 uV drawn inside a ±60 uV hint
  assert.notEqual(traces[0].points, '0,40 7.5,6 15,40');                       // what a hard-coded ±1 clamp would draw
  assert.equal(traces[1].points, '0,120 15,86');                               // ±2 uV drawn inside a ±20 uV hint
  assert.ok(!traces[0].points.includes('51.33,') && !traces[0].points.includes(',51.34'));
  const html = render(decoded({ samples: [[0, 9, -3]], original_scale_hint_uv: 60 }));
...
test('EEG-F06 data above the hint clips visibly at the band edge and says so', () => {
  const html = render(decoded({ samples: [[0, 90, -90]], original_scale_hint_uv: 60, filtered_samples: [[0, 5]], filtered_scale_hint_uv: 20 }));
  const [raw] = tracesOf(html);
  assert.equal(raw.points, '0,40 7.5,6 15,74');                                 // pinned to both edges of the raw band
...
test('EEG-F09 no usable trace keeps the awaiting state and draws nothing', () => {
  for (const stream of [undefined, { timestamp: 0, values: {} }, { timestamp: 0, values: decoded({ samples: null, filtered_samples: null }) },
    { timestamp: 0, values: decoded({ samples: [[0, null]], filtered_samples: [[NaN, 1]] }) }]) {
...
test('EEG-F10 a trace without a declared rate is index-spaced and never claims seconds', () => {
  const html = renderStream({ timestamp: 0, values: { samples: [[0, 1, -1]], channels: ['illustration'] } });
  const traces = tracesOf(html);
  assert.equal(traces.length, 1);
...
test('EEG-F11 a full 5 s packet at two different rates lands on one shared grid in the dashboard', () => {
  ...
  const payloads = [
    ...
    { version: 1, sequence: 2, type: 'eeg_display', source: 'nova-aad', session_id: 's1', timestamp: 0.5,
      payload: { ...payload(), samples: [raw], filtered_samples: [filtered], filtered_sample_rate: 32 } },
  ];
  ...
  assert.equal(rawPoints[1][0] - rawPoints[0][0], 7.5);        // 1/16 s of a 5 s window across 600 units
  assert.equal(filteredPoints[1][0] - filteredPoints[0][0], 3.75); // 1/32 s: its own rate, not sample_rate
