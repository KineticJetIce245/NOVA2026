  ...
  return h('main', { className: `dashboard${mediaConfig ? ' audio-session' : ''}` },
    ...
    h(mediaConfig ? 'details' : 'div', { className: mediaConfig ? 'additional-status' : undefined },
      ...
      h(StatusCard, { title: 'System status', rows: [
        ...
        ['EEG hardware status', 'Unavailable'], ['Presentation', latest('presentation')?.values.mode], ['Data source', latest('presentation')?.values.sourceMode], ['Left ear', latest('presentation')?.values.left], ['Right ear', latest('presentation')?.values.right], ['Sync offset', displayValue(latest('sync')?.values.offsetMs, ' ms')], ['Synchronization', latest('sync')?.values.status], ['Session', session],
      ] }),
    ...
    h('details', { className: 'card diagnostics' }, h('summary', null, 'Stream diagnostics · decoded results'), ...state.streams.filter(stream => stream.type !== 'feedback').map(stream => h('section', { key: JSON.stringify([stream.source, stream.type]) },
      ...
      stream.type === 'prediction' ? h(React.Fragment, null,
        ...
        h('p', null, `${stream.known ? 'Registered decoder' : 'Unknown packet fallback'} · ${stream.simulated === true ? 'SIMULATED' : stream.simulated === false ? 'RECORDED / LIVE SOURCE' : 'Simulation status unavailable / not declared'} · t=${stream.timestamp} · seq=${stream.sequence}`),
