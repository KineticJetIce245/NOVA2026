...
export const decoders = new Map([
  ...
  ['presentation', p => ({ mode: ['dichotic', 'diotic'].includes(p.mode) ? p.mode : null, left: text(p.left), right: text(p.right), ...(Object.hasOwn(p, 'source_mode') ? { sourceMode: text(p.source_mode) } : {}) })],
  ['gain', p => ({ a_db: number(p.a_db), b_db: number(p.b_db), ...mediaReference(p) })],
  ['signal_quality', p => ({ quality: number(p.quality), artifact: boolean(p.artifact), ...(Array.isArray(p.bad_channels) ? { badChannels: p.bad_channels.filter(c => typeof c === 'string') } : {}) })],
  ['feedback', p => ({
  ...
  ['sync', p => ({ status: text(p.status), offsetMs: number(p.offset_ms), driftWarning: boolean(p.drift_warning),
    ...
    ...(Object.hasOwn(p, 'clock_domain') ? { clockDomain: text(p.clock_domain), maxBlockTimingErrorSeconds: number(p.max_block_timing_error_seconds), estimatedAudioDriftPpm: number(p.estimated_audio_drift_ppm) } : {}) })],
  ['eeg_display', p => ({
